...
import Clock  from "../contracts/Clock.json";
...
class Connect extends Component {
  ...
  async getContractsInstances(){
    ...
    this.BlockClockInstance = new this.web3.eth.Contract(
      Clock.abi,
    ...
    this.context.updateAccountInfo({BlockClockInstance: this.BlockClockInstance})
    this.context.updateAccountInfo({ClockMintInstance: this.ClockMintInstance})
    let currentBlock = await this.web3.eth.getBlockNumber()
    this.context.updateAccountInfo({currentBlockNumber: currentBlock})
    this.getMintInfo();
    this.getClockURI();
  }

  async getClockURI(){
    if(this.context.clockIdPath !== null){
        let URI = await this.context.BlockClockInstance.methods.tokenURI(this.context.clockIdPath).call()
        const URIData = await fetch(URI)
        const URIDataJSON = await URIData.json();
        this.context.updateAccountInfo({clockURI: URIDataJSON})
    }
    setTimeout(this.context.getClockURI, 15000);
  }
